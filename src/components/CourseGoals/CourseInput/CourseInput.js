...
const CourseInput = (props) => {
  ...
  return (
    <form onSubmit={formSubmitHandler}>
      <div className={`${styled['form-control'] } ${!isValid && styled.invalid}`}> {/*we can use props to achieve a dynamic css control  */}
        <label>Course Goal</label>
        <input type="text" onChange={goalInputChangeHandler} />
      </div>
      <Button type="submit">Add Goal</Button> {/* this is Genius*/}
    </form>
  );
};
